...
test_pascal(P,TestFolds,LL,AUCROC,ROC,AUCPR,PR):-
  ...
  write('dopo test_prob_pascal'),nl,
  write(P),nl,
  write('folds:'),nl,
  write(TestFolds),nl,
  write('log likelihood:'),nl,
  write(LL),nl,
  write('lg:'),nl,
  write(LG),nl,
...
test_prob_pascal(M:P,TestFolds,NPos,NNeg,CLL,Results) :-
  write2(M,'Testing\n'),
  write('Inizio di test_prob_pascal'), nl,
  write('TestFolds: '), write(TestFolds), nl,
  write('P: '), write(P), nl,
  findall(Exs,(member(F,TestFolds),M:fold(F,Exs)),L),
  append(L,TE),
  write('Esempi totali raccolti da TestFolds: '), write(TE), nl,
  test_no_area(TE,P,M,NPos,NNeg,CLL,Results),
  % Stampa dei risultati ottenuti
  write('Risultati di test_prob_pascal'), nl,
  write('NPos: '), write(NPos), nl,
  write('NNeg: '), write(NNeg), nl,
  write('CLL: '), write(CLL), nl,
  write('Results: '), write(Results), nl.

test_no_area(TestSet,P0,M,NPos,NNeg,CLL,Results):-
write('test_no_area - Inizio'), nl,
write('TestSet: '), write(TestSet), nl,
rule_to_int(P0, P),
write('P dopo rule_to_int: '), write(P), nl,

test_ex(TestSet, P, M, Results, 0, NPos, 0, NNeg, 0, CLL),

write('test_no_area - Fine'), nl,
write('NPos: '), write(NPos), nl,
write('NNeg: '), write(NNeg), nl,
write('CLL: '), write(CLL), nl,
write('Results: '), write(Results), nl.
