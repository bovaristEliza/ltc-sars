...
compute_areas(LG,AUCROC,ROC,AUCPR,PR):-
  ...
  length(Neg,NNeg),
  NPos is NEx-NNeg,
  %write('Numero esempi: '), write(NEx), nl,
  %write('Numero esempi positivi: '), write(NPos), nl,
  %write('Numero esempi negativi: '), write(NNeg), nl,
  keysort(LG,LG1),
  reverse(LG1,LG2),
  ...
  %write('PR: '), write(PR), nl,
  %write('compute_areas - Fine'), nl.
...
compute_pointsroc([],_P0,_TP,_FP,_FN,_TN,P0,P1):-!,
  ...
  %write('compute_pointsroc - Caso Base: '), nl.
...
compute_aucpr(L,Pos,Neg,A,PR):-
  ...
  %writeln('compute_aucpr inizio'),
  %write('P_0: '), writeln(P_0),
  %write('FP: '), writeln(FP),
  %write('TP: '), writeln(TP),
  %write('FN: '), writeln(FN),
  %write('TN: '), writeln(TN),
