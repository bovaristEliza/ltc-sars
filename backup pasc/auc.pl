...
compute_areas(LG,AUCROC,ROC,AUCPR,PR):-
  ...
  keysort(LG,LG1),
  reverse(LG1,LG2),
  write('LG: '),write(LG),nl,
  write('LG1: '),write(LG1),nl,
  write('LG2: '),
  write(LG2),nl,
  write('NEx: '),write(NEx),nl,
  write('NPos: '),write(NPos),nl,
  write('NNeg: '),write(NNeg),nl,
...
write('compute_areas_diagrams'),nl,
write(LG),nl,
  must_be(list, LG),
  must_be(var, AUCROC),
  must_be(var, ROC),
...
write('compute_pointsroc dentro 0'),nl,
write('P0: '),write(P0),nl,
write('P1: '),write(P1),nl,

  append(P0,[1.0-1.0],P1).

compute_pointsroc([P- (\+ _)|T],P0,TP,FP,FN,TN,Po0,Po1):-!,
write('compute_pointsroc dentro 1'),nl,
  (P<P0->
    FPR is FP/(FP+TN),
    TPR is TP/(TP+FN),
  ...
  write('Po0: '),write(Po0),nl,
  write('Po2: '),write(Po2),nl,
  write('P1: '),write(P1),nl,
  write('FPR: '),write(FPR),nl,
  write('TPR: '),write(TPR),nl,
  FP1 is FP+1,
  TN1 is TN-1,
  compute_pointsroc(T,P1,TP,FP1,FN,TN1,Po2,Po1).

compute_pointsroc([P- _|T],P0,TP,FP,FN,TN,Po0,Po1):-!,
write('compute_pointsroc dentro 2'),nl,
  (P<P0->
    FPR is FP/(FP+TN),
    TPR is TP/(TP+FN),
    append(Po0,[FPR-TPR],Po2),
...
write('compute_aucpr'),nl,
  ...
  write('Neg: '),write(Neg),nl,
   write('TL: '),write(TL),nl,
  write('P_0: '),write(P_0),nl,
  write('TP: '),write(TP),nl,
  write('FP: '),write(FP),nl,
  write('FN: '),write(FN),nl,
  write('TN: '),write(TN),nl,
  compute_curve_points(TL,P_0,TP,FP,FN,TN,Points),
  write('compute_curve_points'),nl,
